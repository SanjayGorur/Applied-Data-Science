...
if __name__ == "__main__":
    ...
    #net = buildNetwork(len(t), 40, 1, outclass=SoftmaxLayer)
    net = buildNetwork(len(t), .04*len(t), 1)
    #net = buildNetwork(len(t),.04*len(t), .04*len(t),1)
    trainer = BackpropTrainer( net, ds)
    #trainer = BackpropTrainer( net, ds, momentum=0.1, learningrate=0.01 ,  weightdecay=0.01)
    error = 10
    iteration = 0
    while error > 0.0001: 
        error = trainer.train()
